...
async function readState(env) {
  return {
    ...
    cover: (await env.PRICES.get("cover", "json")) || null,
  };
}
...
async function writeState(env, state) {
  await Promise.all([
    ...
    env.PRICES.put("cover", JSON.stringify(state.cover || null)),
  ]);
}
  ...
  async fetch(request, env) {
    ...
    if (path === "/api/publish" && request.method === "PUT") {
      ...
      try {
        ...
        const versions = await readVersions(env);
        return json({ ok: true, overrides: next.overrides, hidden: next.hidden, names: next.names, orders: next.orders, group_names: next.group_names, group_layout: next.group_layout, cover: next.cover, versions }, 200, origin);
      } catch (e) {
        return json({ ok: false, error: String(e) }, 500, origin);
      }
    }
